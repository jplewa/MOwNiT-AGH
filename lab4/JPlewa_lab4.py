...
def ex3():
    ...
    with open('csv/ex3.csv', 'r') as csvfile:
    ...
    ax.legend(loc=9)
    plt.savefig("plot/ex3.png")


def ex4():
    raw_data_y = []
    raw_data_x = []
    with open('csv/ex4_in.csv', 'r') as csvfile:
        for row in csv.reader(csvfile, delimiter=','):
            if row[0] == "padding":
                break
    ...
    plt.savefig("plot/ex4_data.png")
    re = []
    im = []
    with open('csv/ex4_out.csv', 'r') as csvfile:
        for row in csv.reader(csvfile, delimiter=','):
...
def ex5():
    ...
    fftw = []
    _, ax = plt.subplots(figsize=(10, 6))
    with open('csv/ex5.csv', 'r') as csvfile:
        for row in csv.reader(csvfile, delimiter=','):
            N.append(int(row[0]))
