...
def comparison():
    expected = {}
    with open('csv/JPlewa_comparison_expected.csv', 'rt') as csvfile:
    ...
    with open('csv/comparison.csv', 'rt') as csvfile:
        for row in csv.reader(csvfile, delimiter=','):
    ...
    for fun, methods in ys.items():
        ...
        ax.legend()
        plt.savefig(f'plot/{fun}.png')


def ex3():
    xs = []
    ys = []
    with open('csv/ex3.csv', 'rt') as csvfile:
        for row in csv.reader(csvfile, delimiter=','):
